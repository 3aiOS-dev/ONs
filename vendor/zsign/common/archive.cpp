...
bool Zip::_WriteFileToZip(void* hZip, const string& strFile, const string& strRelativePath, int zip_level)
{
	...
	// At level 0, store files without deflate. IPA payloads are normally
	// already compressed, so avoiding recompression reduces signing time.
	const int method = (zip_level == 0) ? 0 : Z_DEFLATED;
	if (ZIP_OK != zipOpenNewFileInZip3_64(hZip, strRelativePath.c_str(), &zi, NULL, 0, NULL, 0, NULL, method, zip_level, 0, -MAX_WBITS, DEF_MEM_LEVEL, 0, NULL, 0, 0)) {
		fclose(fp);
	...
	bool bRet = true;
	// IPA repackaging dominates wall time for large apps. A 64 KiB buffer
	// reduces file-system calls substantially while remaining stack-safe on iOS.
	char buffer[64 * 1024];
